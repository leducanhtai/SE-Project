...
class AiScoringService
{
    public function scoreAndStore(array $data)
    {
        ...
        $this->updateSubmissionScores($submission, $totalScore, $overallFeedbacks);
        $this->storeDetailedFeedback($submission->id, $allFeedback);

        return ['submission' => $submission->load('feedbacks')];
    }

    /**
     * Cập nhật điểm tổng hợp và phản hồi cho submission
     */
    private function updateSubmissionScores(WritingSubmission $submission, array $totalScore, array $overallFeedbacks): void
    {
    ...
    /**
     * Lưu chi tiết phản hồi vào bảng feedbacks
     */
    private function storeDetailedFeedback(int $submissionId, array $allFeedback): void
    {
