<aside class="w-[260px] bg-[#FF9500] text-white p-5 flex flex-col gap-[30px]">
  <nav class="flex flex-col gap-4 mt-[103px]">
    <a href="{{ route('dashboard') }}" class="{{ request()->routeIs('dashboard') ? 'bg-white text-[#ffb700]' : 'text-white hover:bg-[#ffb700]' }} flex items-center gap-2 px-3 py-2 rounded-lg font-medium transition-colors">
    ...
    <a href="{{ route('writing.test.part') }}" class="{{ request()->routeIs('writing.test.part') ? 'bg-[#889cbca8] text-white' : 'text-white hover:bg-[#ffb700]' }} flex items-center gap-2 px-3 py-2 rounded-lg font-medium transition-colors">
      <img src="{{ asset('icon/practice.svg') }}" alt="" class="{{ request()->routeIs('writing.test.part') ? 'filter brightness-0 invert' : '' }}"> Luyện tập
    </a>
